...
#### Scenario: 방 생성 성공
- **WHEN** 유저가 방 이름과 닉네임을 입력하고 생성 버튼을 누르면
- **THEN** `POST /api/rooms` 요청이 `{ name, visibility, game_type: "mafia", max_humans: 1 }` body와 `X-Player-Name` 헤더로 전송된다
- **THEN** 응답의 `player_id`가 localStorage에 `player_id_{room_id}` 키로 저장된다
- **THEN** 응답의 `id`를 이용해 `/rooms/:id` 대기실 화면으로 이동한다

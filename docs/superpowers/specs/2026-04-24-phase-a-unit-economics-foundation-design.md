...
- **토큰·발화 파라미터 — 용도별 분리** (장애 방지):
  현재 `agent.go:289` 에 `MaxTokens: 300` 이 **모든 용도에 단일값**으로 하드코딩되어 있다. 이로 인해 투표/킬/조사 같은 **짧은 ID 만 기대하는 응답**에서도 300 토큰이 할당되어, LLM 이 reasoning 을 길게 쓰면 실제 ID 가 잘려 나갈 수 있다(→ `containsID` 검증 실패 → AI 투표 기권). 이를 용도별로 쪼갠다:
  - `max_tokens_chat` (int, 기본 160) — 일상 대화·토론 발화
  - `max_tokens_decision` (int, 기본 20) — 투표/킬/조사 (ID 한 개만 받으면 충분)
  - `agent.go` 의 `callLLM` 을 `callLLMChat` / `callLLMDecision` 으로 분리하거나, `callLLM(ctx, model, kind, extraInstruction)` 시그니처로 확장
- **`stop_reason` 처리** — Anthropic 응답의 `stop_reason` 이 `"max_tokens"` 이면:
  - `logger.Warn("llm: truncated", fields...)` 기록
  - `game_metrics.truncated_turns` 카운터 +1 (관측 지표, §3-D 참조)
  - 응답 자체는 그대로 계속 사용. 이미 페일세이프(`containsID` 검증)가 다음 단계에서 걸러줌
...
CREATE TABLE game_metrics (
    ...
    truncated_turns         INT NOT NULL DEFAULT 0,  -- stop_reason == "max_tokens" 발생 횟수
...
### 5.1 AI 응답 장애 시나리오 (핵심 원칙: 게임 로직은 절대 중단되지 않는다)

현재 `agent.go` 는 모든 LLM 응답을 **검증 후 사용** 하는 페일세이프 구조로 되어 있다. 어떤 장애가 발생해도 **해당 AI 턴이 skip 될 뿐 게임은 다음 phase 로 계속**. 아래 표가 그 보장을 명시한다.

| 장애 시나리오 | Anthropic 응답 | `callLLM` 반환 | `agent.go` 검증 | 게임 엔진 영향 | 허용 여부 |
|-------------|--------------|--------------|--------------|------------|:----:|
| API 네트워크/인증 실패 | error | `""` | `targetID == ""` → skip | 투표 없음 → `len(tally) == 0` → 다음 phase | ✅ |
| Rate limit / 5xx | error | `""` | 동일 | 동일 | ✅ |
| 빈 응답 (content[]) | `stop_reason: end_turn` 이나 내용 비어있음 | `""` | 동일 | 동일 | ✅ |
| **`max_tokens` 로 잘림 (채팅)** | `stop_reason: max_tokens` + 부분 텍스트 | 잘린 문자열 | (없음 — 채팅은 그냥 broadcast) | UX 약간 어색, 게임 흐름 영향 없음 | ✅ |
| **`max_tokens` 로 잘림 (투표/킬/조사)** | `stop_reason: max_tokens` + 잘린 ID | 잘린 ID 문자열 | `containsID(alive, targetID) == false` → skip | 해당 AI 기권. 게임은 계속 | ✅ |
| 모델이 유효하지 않은 ID 반환 (환각) | `stop_reason: end_turn` + "player-999" | 유효 안 한 ID | `containsID` false → skip | 해당 AI 기권. 게임은 계속 | ✅ |
| 모든 AI 가 동시에 기권 | — | — | 모든 AI skip | `tally` 비어있음 → 처형 없음 → 다음 phase | ✅ |

**Phase A 개선 (페일세이프는 유지, 가시성 + 예방 추가):**

1. **관측:** `stop_reason == "max_tokens"` 를 `logger.Warn` + `truncated_turns` 카운터 증분 (§3-A, §3-D)
2. **예방:** `max_tokens_decision = 20` 으로 ID 응답 시 토큰 잘림 가능성을 사실상 0 으로 (§3-A). 즉 잘림은 chat 에서만 일어날 여지 — chat 잘림은 UX 이슈지 장애 아님
3. **한계값 모니터링:** Phase A 의 첫 시운전에서 `truncated_turns` 가 예상보다 높으면 `max_tokens_chat` 을 160→200 으로 상향 조정 (config 변경만)
...
| Rate limit 초과 (`/api/metrics/ad`) | 429. 프론트는 silent (impression 은 fire-and-forget) |
| Redis 다운 (rate limiter storage) | limiter middleware 가 fail-open 설정 — Redis 조회 실패 시 요청 통과(로그 남김). 수익 지표 일시 교란 허용, 사용자 UX 유지 |
...
| Postgres 다운 (`game_metrics` 쓰기) | 위 "`GameMetricsRepo` 쓰기 실패" 와 동일 — warn log, 게임 계속 |

**공통 원칙:** 모든 측정/수익 경로는 **fail-open** (장애 시 사용자 게임 흐름을 막지 않는다). 모든 게임 핵심 경로는 **fail-skip** (개별 AI 턴 포기 후 다음 phase).
...
| 장애 상황에서 게임 진행 보장 | `max_tokens` 강제 시에도 게임 흐름 계속 | `max_tokens_decision=1` 으로 강제 truncation → 모든 AI 투표 skip → `phases_test.go` 에서 게임이 다음 phase 로 진행되는지 검증 |
...
3. ~~`max_tokens_per_turn` 기본값 160~~ — **해소됨(pass 3):** 단일 값 대신 `max_tokens_chat=160` / `max_tokens_decision=20` 으로 용도별 분리. Decision 은 ID 만 받으면 되므로 잘릴 염려 거의 없음. Chat 은 첫 시운전 후 `truncated_turns` 보고 필요 시 상향 조정 (config 변경만)
...
| 2026-04-24 | pass 3 — 장애·토큰 한계 분석 | `max_tokens_per_turn` 단일값을 용도별 분리: `max_tokens_chat=160`, `max_tokens_decision=20`. 이유: 현재 300 단일값은 투표 ID 같은 짧은 응답에서도 LLM 이 reasoning 을 길게 쓰면 실제 ID 가 잘려 `containsID` 검증 실패 → AI 기권. Decision 을 20 으로 잡으면 잘릴 여지 사실상 0. §5 "Error Handling" 을 8행 표로 확장하여 **모든 AI 응답 장애 시나리오가 페일세이프(skip) 경로로 수렴함**을 명시. 게임 로직은 절대 중단되지 않음을 스펙 레벨 보장. `stop_reason == "max_tokens"` 관측을 위해 `game_metrics.truncated_turns` 컬럼 추가. Redis 다운(limiter storage) 시 fail-open 정책 명시. Success Criteria 에 "장애 상황에서 게임 진행 보장" 검증 row 추가 |
